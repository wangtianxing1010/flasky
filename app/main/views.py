...
def edit_profile_admin(id):
	...
	if form.validate_on_submit():
		...
		return redirect(url_for('.user', username=user.username))#why username=user.username Because username is the argument taken by endpoint function user()
...
def post(id):
	...
	pagination=post.comments.order_by(Comment.timestamp.asc()).paginate(page, current_app.config['FLASKY_POSTS_PER_PAGE'],error_out=False)
	comments=pagination.items
	return render_template('post.html',posts=[post],form=form,comments=comments,pagination=pagination)
	
@main.route('/editpost/<int:id>')
@login_required
def edit(id):
	post=Post.query.get_or_404(id)
...
def followers(username):
	...
	return render_template('follows.html',title='Followers of ',user=user,page=page,pagination=pagination,follows=follows)
...
def followed_by(username):
	...
	pagination=user.followed.paginate(page,per_page=current_app.config['FLASKY_FOLLOWERS_PER_PAGE'],
											error_out=False)
	follows=[{'user':item.follwed,'timestamp':item.timestamp} for item in pagination.items]
	return render_template('follows.html',title='Followed by ',user=user,page=page,pagination=pagination,follows=follows)
